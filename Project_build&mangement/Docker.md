...
这是Docker中持久化数据的最好方式，存于主机文件系统中`/var/lib/docker/volumes/`。非Docker进程不应该修改这些数据。
...
存于主机文件系统中的任意位置。非Docker进程可以修改这些数据
...
顾名思义是一种非持久化的数据存储。它仅仅将数据保存在宿主机的内存中，一旦容器停止运行，tmpfs mounts会被移除，**从而造成数据丢失**。

```
docker run -itd --mount type=tmpfs,target=/test 
```
